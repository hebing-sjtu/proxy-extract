...
有两条,产出的短片**逐字节同构**,下游分不出来。区别只在要不要顺带产出长段:

| | `make clips`（两步） | `make clip-episodes`（一趟） |
| --- | --- | --- |
| 前提 | 长段已经交付好 | 只要语料 |
| 模型算多少帧 | 整段 ~1800 帧 | 只算留下的 ~640 帧 |
| target 怎么来 | 1920×1080 → 1280×720 → **上采** 1344×768 | 1920×1080 → 1344×768，只缩一次 |
| 长段 | 有 | 没有 |
| 主角判定范围 | 整段 | 每片自己的 124 帧 |

**长段本身要交付,就走两步。** 交付都要跑,从里面切等于白捡,`make clip-episodes` 反而
是对同一份语料的第二遍。

**只要短片,就走一趟。** 模型少算 2.8 倍(2000 段是 360 万帧降到 128 万帧),而且省掉
那次上采——1280×720 上采到 1344×768 补不回降采时丢掉的细节。

```bash
make clip-episodes LIMIT=4      # 先在 4 条上看
make clip-episodes
make clips-audit
```

一趟这条会为每片建一个临时的 `.work/`(该窗口的逐帧预测,约 580 MiB),编完就删。
`--keep-work` 可以留下来查。

**每个窗口是一条独立的小 episode**,这不只是省算力。`split_people` 是按 blob 连轨迹
的,五个窗口相隔几分钟,跨窗口跑它会把上一窗末尾的人和下一窗开头的另一个人连成一条
轨迹。按窗口判,主角是从这一片自己的帧里判出来的——对训练样本来说本来就该是这个范围。
代价是同一条 episode 的五片可能选出不同的主角,`clip_report.json` 里每片都记了自己的
`hero_split`。

窗口两侧还会多算几帧(`--halo`,默认等于 `--temporal-radius`),因为稳定器要看每帧左右
各若干帧。没有这几帧的话,每片的头尾几帧会比中间少一半上下文——一万片在同一个位置都有
这道缝,模型会去学它而不是忽略它。这些帧算完就丢。
